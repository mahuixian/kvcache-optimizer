...
class AttentionFactory:
    def create_attention(self, item, args):
        if item == "H2O":
            return H2OAttention(args)
        else:
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        if seqlen > 1:
            ...
            mask = torch.hstack([torch.zeros((seqlen, past_len)), mask]).type_as(tokens_embeddings)
