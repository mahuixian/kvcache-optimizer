...
class Llama: 
    ...
    def generate(self, 
        ...
        """
        bsz = len(input_ids)
        assert bsz <= self.params.max_batch_size, f"Batch size {bsz} exceeds max batch size {self.params.max_batch_size}"
        min_prompt_len = min([len(x) for x in input_ids])
        max_prompt_len = max([len(x) for x in input_ids]) 
        assert max_prompt_len <= self.params.max_seq_len, f"Prompt length {max_prompt_len} exceeds max sequence length {self.params.max_seq_len}"
        total_len = min(self.params.max_seq_len, max_gen_len + max_prompt_len) #min(训练长度, 生成长度+输入最大长度)
